...
const BuildingList = ({ setRPS }: { setRPS: (value: number) => void }) => {
    ...
    <ul className={"ul-horizontal"} key={this}>
      {
        playerInfo?.["building"] && playerInfo["building"].map((building, index) => (
          <Building key={index} building={building} imgPath={getImgPath(index, building.price ?? 0)} index={index} />
        ))
      }
    </ul>
  );
...
const Building = ({ building, imgPath, index }: BuildingProps) => {
  ...
  function enforceMinMax(el: ChangeEvent<HTMLInputElement>) {
    if (Number(building.name) === -1) return;
    ...
    if (value) {
      ...
      let targettedBuilding = playerInfo.building.find((m) => m.name === building.name)!;

      targettedBuilding = {
        ...targettedBuilding,
        own: parseInt(el.target.value),
      }

      const buildings = playerInfo.building.filter((m) => m.name !== building.name).concat(targettedBuilding);
      setPlayerInfo({ ...playerInfo, building: buildings });
    }
  }


  function printPricePretty(price: string) {
    ...
    <li>
      <ul className={"Info-Building-list " + (building["own"] !== 0 ? "Owned" : "NotOwned")}>
        ...
        <div className={"imageWrapper"}>
          <img src={import.meta.env.VITE_PUBLIC_URL + "/" + imgPath} alt="image" className={"Building-img"}></img>
        ...
        <input type="number" min="0" step="1" max="99" value={building["own"]}
