...
async function decryptAES256(encryptedText: string, key: string): Promise<string> {
  ...
  const encryptedDataWithIv = Uint8Array.from(atob(encryptedText), c => c.charCodeAt(0));
...
export const getNewQuestionPalaAnimation = async (username: string): Promise<{
  ...
  const response = await axios.get<string>(`${API_PALATRACKER_URL}/v1/palaAnimation/question?username=${username}`, {
    timeout: 4000
  }).catch((error) => error);
