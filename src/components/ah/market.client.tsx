...
export default function MarketPage() {
  ...
  const axisPressureConfigs: AxisConfig[] = [
    { id: "x-axis", position: "bottom", type: "date", color: "#9ca3af" },
  ...
  const { priceVolumeDatasets, pressureDatasets } = useMemo(() => {
    const priceDs: Dataset<Date, number> = {
      id: "price", name: "Prix ($)", color: "#22c55e", visibility: true, yAxisId: "left",
    ...
    const pressureDs: Dataset<Date, number> = {
      id: "pressure", name: "Pression", color: "#a855f7", visibility: true, yAxisId: "left",
      ...
      <Card>
        <>
          <section className={"grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500"}>
            <div className="lg:col-span-1 bg-gray-900/50 border border-gray-800 rounded-xl p-6 flex flex-col items-center justify-center space-y-4 h-full relative">
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogTrigger asChild>
                  <div
                    className="w-32 h-32 bg-gray-800/50 rounded-full flex items-center justify-center border-2 border-gray-700 mb-2 relative overflow-hidden group shadow-lg shadow-orange-900/10 cursor-pointer hover:border-orange-500 transition-colors"
              ...
              <span className="text-xs text-gray-500 bg-gray-800 px-2 py-1 rounded">{displayItem.label2}</span>

              {currentStats && (
                <div className="w-full grid grid-cols-2 gap-3 mt-2">
                  <div className="bg-gray-800/60 p-3 rounded-lg border border-gray-700 text-center flex flex-col justify-between">
                    <span className="text-[10px] text-gray-400 uppercase tracking-wide">Prix Actuel</span>
                  ...
                  <div className="bg-gray-800/60 p-3 rounded-lg border border-gray-700 text-center flex flex-col justify-center">
                    <span className="text-[10px] text-gray-400 uppercase tracking-wide">Ventes du jour</span>
              ...
              <Link
                ...
                className={"w-full mt-4 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white py-2.5 rounded-lg font-semibold transition-all shadow-md shadow-indigo-900/20 active:scale-95 group/btn"}
              >
            ...
            {currentListings.length > itemsPerPage && (
              <div className="mt-8 flex items-center justify-center gap-4">
                <button
                  ...
                  className="p-2 rounded-lg bg-gray-800 border border-gray-700 text-gray-300 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  <ChevronLeft size={20} />
                </button>
