...
export function ProfileFetcherWrapper({ username, children }: {
  ...
  const { data: playerInfo, setPlayerInfo, setDefaultProfile } = usePlayerInfoStore();
  const { settings } = useSettingsStore();

  const [isFirstRender, setIsFirstRender] = useState(true);
  const [alreadyCheckedProfile, setAlreadyCheckedProfile] = useState(false);
  const [isLoading] = useState(false);
  const defaultProfileTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    if (playerInfo !== null) {
      if (defaultProfileTimeoutRef.current) {
        clearTimeout(defaultProfileTimeoutRef.current);
        defaultProfileTimeoutRef.current = null;
      }
      return;
    }

    defaultProfileTimeoutRef.current = setTimeout(() => {
      setDefaultProfile();
    }, 500);
  }, [alreadyCheckedProfile, playerInfo, setDefaultProfile]);
