...
import type { Metier, MetierKey, Metiers } from "@/types";
...
const MetierList = ({ editable = true }: MetierListProps) => {
  ...
  // Convert metier to a list, Sort metiers, first mineur, then farmer, then hunter, then alchimiste
  const metiers = Object.values(playerInfo.metier).sort((a, b) => {
      ...
      {metiers.map((metier, index) => (
        <Card key={index}>
          <CardContent className="pt-6 flex flex-col items-center justify-center gap-2">
            <MetierComponent key={metier.name}
