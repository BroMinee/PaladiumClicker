...
test.skip("Test Graph element", async ({ page }) => {
  await page.goto("http://localhost:3000/ah?item=paladium-ingot");
  await expect(page.locator("#graph-historic-plot .recharts-wrapper svg:nth-child(3)")).toBeVisible();
  await expect(page.locator("#curve-price").first()).toBeVisible();
  await expect(page.locator("#curve-price").nth(0)).toBeVisible();
  await expect(page.locator("#curve-quantity").first()).toBeVisible();
  await expect(page.locator("#curve-quantity").nth(0)).toBeVisible();
});
...
test.skip("Test current sells", async ({ page }) => {
  await page.goto("http://localhost:3000/ah?item=paladium-ingot");
